    ...
    if ($_GET["getAllQueries"] !== "empty") {
        ...
        if (!is_bool($results)) {
            while ($row = $results->fetchArray()) {
                ...
                $allQueries[] = array($row[0], $query_type, utf8_encode($row[2]), utf8_encode($c), $row[4], utf8_encode($row[5]));
            }
        }
    }
...
if (isset($_GET["topDomains"]) && $auth) {
    ...
    $stmt = $db->prepare(
        "SELECT domain,count(domain) FROM queries WHERE (STATUS == 2 OR STATUS == 3)" .
            $limit .
            " GROUP by domain order by count(domain) desc limit 20"
    );
    $stmt->bindValue(":from", intval($_GET["from"]), SQLITE3_INTEGER);
    $stmt->bindValue(":until", intval($_GET["until"]), SQLITE3_INTEGER);
    $results = $stmt->execute();
...
if (isset($_GET["topAds"]) && $auth) {
    ...
    $stmt = $db->prepare(
        "SELECT domain,count(domain) FROM queries WHERE (STATUS == 1 OR STATUS > 3)" .
            $limit .
...
if (isset($_GET["getGraphData"]) && $auth) {
    ...
    $stmt = $db->prepare(
        "SELECT (timestamp/:interval)*:interval interval, COUNT(*) FROM queries WHERE (status != 0 )" .
            $limit .
    ...
    $stmt = $db->prepare(
        "SELECT (timestamp/:interval)*:interval interval, COUNT(*) FROM queries WHERE (status == 1 OR status > 3)" .
            $limit .
            " GROUP by interval ORDER by interval"
    );
    $stmt->bindValue(":from", $from, SQLITE3_INTEGER);
