...
{
    // Check CSRF token
    $session_started = function_exists("session_status") ? session_status() == PHP_SESSION_ACTIVE : session_id() == "";
...
function list_verify($type)
{
    ...
    if (isset($_POST["token"])) {
        ...
        if ($wrongpassword || !$auth) {
            log_and_die(
                "Wrong password - " . htmlspecialchars($type) . "listing of " . htmlspecialchars($_POST["domain"]) . " not permitted"
            );
        }
    } else {
