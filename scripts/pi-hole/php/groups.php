    ...
    try {
        ...
        if ($_POST["action"] == "replace_domain") {
            ...
            $check_stmt = $db->prepare(
                "SELECT EXISTS(SELECT domain FROM domainlist_by_group dlbg JOIN domainlist dl on dlbg.domainlist_id = dl.id WHERE dl.domain = :domain AND dlbg.group_id != 0)"
            );
            if (!$check_stmt) {
        ...
        if (!$insert_stmt->bindValue(":type", $type, SQLITE3_TEXT) || !$update_stmt->bindValue(":type", $type, SQLITE3_TEXT)) {
        ...
        foreach ($domains as $domain) {
            ...
            if ($_POST["action"] == "replace_domain") {
                if (!$check_stmt->bindValue(":domain", $domain, SQLITE3_TEXT)) {
                    throw new Exception(
                        "While binding domain to check: <strong>" .
                            $db->lastErrorMsg() .
                            "</strong><br>" .
                            "Added " .
                            $added .
                ...
                if (!$only_default_group) {
                    throw new Exception(
                        "Domain " .
                            $domain .
                            " is configured with special group settings.<br>" .
            ...
            if (!$insert_stmt->bindValue(":domain", $domain, SQLITE3_TEXT) || !$update_stmt->bindValue(":domain", $domain, SQLITE3_TEXT)) {
                throw new Exception(
                    "While binding domain: <strong>" .
                        $db->lastErrorMsg() .
                        "</strong><br>" .
            ...
            if (!$insert_stmt->execute()) {
                throw new Exception(
                    "While executing INSERT OT IGNORE: <strong>" .
                        $db->lastErrorMsg() .
                        "</strong><br>" .
                        "Added " .
                        $added .
